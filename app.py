...
# 1. Get exact columns and filter out engineered ones
...
# 2. Smartly group features into 4 Categories
demo_keywords = ['age', 'gender', 'residence', 'married'] # Removed 'work' from here
vital_keywords = ['glucose', 'bmi', 'hypertension', 'heart']
work_keywords = ['work'] # New Group for Work Type
...
# 3. Define the Highly Accurate Prediction Function
...
# 4. Helper to create user-friendly UI inputs
def create_friendly_input(col_name):
    if col_name == 'age':
        ...
        return gr.Radio(choices=["0 (Female)", "1 (Male)"], label="Gender", value="0 (Female)")
...
# 5. Custom CSS for Liquid Glass UI
liquid_glass_css = """
body, .gradio-container {
    background: radial-gradient(at 0% 0%, hsla(253,16%,7%,1) 0, transparent 50%),
                radial-gradient(at 50% 0%, hsla(225,39%,30%,1) 0, transparent 50%),
                radial-gradient(at 100% 0%, hsla(339,49%,30%,1) 0, transparent 50%),
                linear-gradient(135deg, #1e0b36 0%, #0c1631 100%) !important;
...
.glass-panel h1, .glass-panel h2, .glass-panel h3, .glass-panel p, .glass-panel span, .glass-panel label {
    color: #ffffff !important;
...
# 6. Build the Layout (Now with 4 Groups and Instruction Notes)
with gr.Blocks(css=liquid_glass_css, theme=gr.themes.Base(neutral_hue="slate")) as app:
    ...
    with gr.Column(elem_classes="glass-panel"):
        ...
        with gr.Row():
            # LEFT SECTION: Beautifully Grouped Input Forms (4 Sections)
            with gr.Column(scale=2):

                with gr.Accordion("👤 1. Patient Demographics", open=True):
                    for col in group_demographics:
                        ui_elements_dict[col] = create_friendly_input(col)

                with gr.Accordion("🩺 2. Vitals & Medical History", open=True):
                    for col in group_vitals:
                        ui_elements_dict[col] = create_friendly_input(col)

                # ---------------------------------------------------------
                # 🚨 NEW GROUP: Work / Employment Status with Special Note
                # ---------------------------------------------------------
                with gr.Accordion("💼 3. Employment Status", open=True):
                    gr.Markdown("""
                    <div style='background: rgba(255, 165, 0, 0.2); padding: 8px; border-radius: 8px; border: 1px solid rgba(255, 165, 0, 0.4); margin-bottom: 10px;'>
                        <b>⚠️ Important Note:</b> Due to data encoding, please select <b>'1 (Yes)' for ONLY ONE</b> of the employment types below. Keep the rest as '0 (No)'.
                    </div>
                    """)
                    for col in group_work:
                        ui_elements_dict[col] = create_friendly_input(col)

                with gr.Accordion("🚬 4. Lifestyle & Other", open=True):
                    for col in group_lifestyle + group_other:
                        ui_elements_dict[col] = create_friendly_input(col)

                submit_btn = gr.Button("🧠 Analyze Patient Risk", variant="primary", size="lg")

            # RIGHT SECTION: Output & Model Info
            with gr.Column(scale=1):
                ...
                gr.Markdown("""
                <br>
                <div style="background: rgba(0,0,0,0.2); padding: 15px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.1);">
                    <h3 style="margin-top: 0;">🤖 Model Architecture Details</h3>
                    <p><b>Primary Engine:</b> Soft-Voting Machine Learning Ensemble</p>
                    <p><b>Base Algorithms Used:</b></p>
                    <ul>
                        <li>⚡ Logistic Regression</li>
                        <li>🌲 Random Forest Classifier</li>
                        <li>🚀 XGBoost Classifier</li>
                    </ul>
                    <p><b>Data Processing:</b> StandardScaler + SMOTE (Engineered Features)</p>
                </div>
                """)

        # Connect the inputs to the function in the exact order of user_cols
        final_input_list = [ui_elements_dict[col] for col in user_cols]

        submit_btn.click(
            fn=predict_stroke,
            inputs=final_input_list,
